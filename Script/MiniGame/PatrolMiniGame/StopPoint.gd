...
func _process(_delta: float) -> void:
	if not is_active: sprite.play("disable")
		
	else: sprite.play("default")
	
	if is_finish: sprite.play("home")
	
	if patrols_going_to_point: sprite.play("patrol")
		
	if Input.is_action_just_pressed("MovePlayer") and State.can_play_sfx: Signals.play_sound.emit(State.AudioType.Effect, button_sfx, 1, -5)
